...
# would be relevant.
@given(arrays(np.float, (3, 5), elements=floats(min_value=0, allow_infinity=False)),
       arrays(np.float, (TEST_LENGTH, 3, 5), elements=floats(min_value=0, max_value=1e30)))
def test_olsen_randerson_gpp_once(flux_gpp, par):
    """Test single downscaling of GPP."""
    assume(np.all(np.any(par != 0, axis=0)))
    flux_gpp_downscaled = olsen_randerson.olsen_randerson_gpp_once(flux_gpp, par)
    ...
    np_tst.assert_allclose(flux_gpp_downscaled_upscaled, flux_gpp * TEST_LENGTH,
                           atol=1e-100)


@given(arrays(np.float, (3, 5), elements=floats(min_value=0, allow_infinity=False)),
       arrays(np.float, (TEST_LENGTH, 3, 5), elements=floats(min_value=-100, max_value=100)))
def test_olsen_randerson_resp_once(flux_resp, temperature):
    ...
    np_tst.assert_allclose(flux_resp_downscaled_upscaled, flux_resp * TEST_LENGTH,
                           atol=1e-100)


@given(arrays(np.float, (3, 5), elements=floats(min_value=-1e30, max_value=+1e30)),
       arrays(np.float, (TEST_LENGTH, 3, 5), elements=floats(min_value=-100, max_value=100)),
       arrays(np.float, (TEST_LENGTH, 3, 5), elements=floats(min_value=0, max_value=1e30)))
def test_olsen_randerson_once(flux_nee, temperature, par):
    ...
    np_tst.assert_allclose(flux_nee_downscaled_upscaled, flux_nee * TEST_LENGTH,
                           atol=1e-100)
